...
SCRIPT_VERSION = "2026-08-24-dh-multiindex-final-fix-v1"
...
def standardize_frame(raw, tool_name, filename):
    ...
    if not detected["state"] or not detected["investigator"]:
        raise ValueError(
            f"{filename}: missing State or Investigator/QDC column. Available headers: {list(raw.columns)}"
        )
    ...
    if level_source:
        ...
        frame["__Level"] = clean_text(frame[level_source]).map(
            lambda v: "DH" if normalize(v) in dh_set else "Below DH"
        )
    else:
...
def create_state_workbook(state, frames, processing_log, output_folder, spoc_map):
    ...
    dh_df = build_dh_below_dh(state_frames, investigators)  # keep raw, handle safely below
    ...
    with pd.ExcelWriter(
        ...
        wb = writer.book
        hf = wb.add_format(
            {"bold": True, "bg_color": "#B7DEE8", "border": 1, "align": "center", "valign": "vcenter", "text_wrap": True}
        )
        ...
        # === FINAL SAFE WRITE FOR DH SHEET ===
        logging.info("SCRIPT_VERSION=%s", SCRIPT_VERSION)
        logging.info("DH columns class before flatten: %s", type(dh_df.columns))

        dh_df = flatten_columns(dh_df)
        logging.info("DH columns class after flatten: %s", type(dh_df.columns))

        # Absolute safety: write with index=True to avoid pandas MI/index=False path completely.
        dh_df.to_excel(writer, sheet_name="DH & Below DH", index=True)
        ws_dh = writer.sheets["DH & Below DH"]
        ws_dh.set_column(0, 0, 6)  # index column
