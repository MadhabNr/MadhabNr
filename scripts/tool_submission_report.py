...
SCRIPT_VERSION = "2026-08-22-force-dh-index-v1"
...
def standardize_frame(raw, tool_name, filename):
    ...
    if detected["submission_date"]:
        ...
        bad = int(frame["__DateTime"].isna().sum())
    else:
...
def load_and_standardize_data(drive_service, source_folder_id):
    ...
    for item in TOOL_FILES:
        t, fn = item["tool_name"], item["filename"]
        logging.info("Reading %s", fn)
        try:
            fid = find_file_id_by_name(drive_service, source_folder_id, fn)
            raw = read_tool_file(download_drive_file_bytes(drive_service, fid), item["reader"])
            frames[t], log = standardize_frame(raw, t, fn)
            logs.append(log)
        except Exception as exc:
            logging.exception("Could not process %s", fn)
            logs.append({"Tool": TOOL_LABELS[t], "File": fn, "Rows_Read": 0, "Status": f"ERROR: {exc}"})
    if not frames:
...
def get_states(frames):
    return sorted({str(v).strip() for f in frames.values() for v in f["__State"].dropna().unique() if str(v).strip()}, key=str.casefold)
...
def get_investigators(state_frames):
    return sorted({str(v).strip() for f in state_frames.values() for v in f["__Investigator"].dropna().unique() if str(v).strip()}, key=str.casefold)
...
def create_state_workbook(state, frames, processing_log, output_folder, spoc_map):
    ...
    with pd.ExcelWriter(output_path, engine="xlsxwriter", engine_kwargs={"options": {"strings_to_urls": False}}) as writer:
        ...
        # FORCE SAFE: if dh_df has MultiIndex columns, index=True avoids pandas NotImplementedError
        logging.info("DH columns type before write: %s", type(dh_df.columns))
        dh_df.to_excel(writer, sheet_name="DH & Below DH", index=True)
        ws_dh = writer.sheets["DH & Below DH"]
        ws_dh.set_column(0, 0, 6)  # index column width
        format_sheet(ws_dh, dh_df, hf, tf, 0, (1, 1), 24)
