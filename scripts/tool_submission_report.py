...
SCRIPT_VERSION = "2026-08-22-multiindex-hardfix-v2"
...
def to_excel_safe(df: pd.DataFrame, writer, sheet_name: str, index: bool = False, startrow: int = 0):
    safe_df = flatten_columns(df)
    # hard fallback: if pandas still thinks MI, force plain Index explicitly
    if isinstance(safe_df.columns, pd.MultiIndex):
        safe_df.columns = pd.Index([" | ".join(map(str, c)) for c in safe_df.columns.to_flat_index()])
    safe_df.to_excel(writer, sheet_name=sheet_name, index=index, startrow=startrow)
    return safe_df
...
def standardize_frame(raw, tool_name, filename):
    ...
    if level_source:
        ...
        frame["__Level"] = clean_text(frame[level_source]).map(lambda v: "DH" if normalize(v) in dh else "Below DH")
    else:
...
def load_and_standardize_data(drive_service, source_folder_id):
    ...
    for item in TOOL_FILES:
        tool_name, filename = item["tool_name"], item["filename"]
        ...
        try:
            ...
            logs.append({"Tool": TOOL_LABELS[tool_name], "File": filename, "Rows_Read": 0, "Status": f"ERROR: {exc}"})
    if not frames:
...
def get_states(frames):
    return sorted(
        {str(v).strip() for f in frames.values() for v in f["__State"].dropna().unique() if str(v).strip()},
        key=str.casefold,
    )


def is_excluded_state(state):
    return normalize_state(state) in {normalize_state(v) for v in EXCLUDED_STATES}


def get_investigators(state_frames):
    return sorted(
        {str(v).strip() for f in state_frames.values() for v in f["__Investigator"].dropna().unique() if str(v).strip()},
        key=str.casefold,
    )
...
def create_state_workbook(state, frames, processing_log, output_folder, spoc_map):
    ...
    with pd.ExcelWriter(output_path, engine="xlsxwriter", engine_kwargs={"options": {"strings_to_urls": False}}) as writer:
        ...
        nurse_df_written = to_excel_safe(nurse_df, writer, "Nurse Wise", index=False)
        format_sheet(writer.sheets["Nurse Wise"], nurse_df_written, hf, tf, 0, (1, 1), 32)

        dh_df_written = to_excel_safe(dh_df, writer, "DH & Below DH", index=False)
        format_sheet(writer.sheets["DH & Below DH"], dh_df_written, hf, tf, 0, (1, 1), 24)

        summary_df_written = to_excel_safe(summary_df, writer, "Summary", index=False, startrow=2)
        ws = writer.sheets["Summary"]
        ws.write(0, 0, f"GAVB Facility Tool Data collection status as on {report_date}", title)
        format_sheet(ws, summary_df_written, hf, None, 2, (3, 1), 28)
        ...
        for item in TOOL_FILES:
            ...
            logging.info("%s: writing %s (%s rows x %s columns)", state, sn, len(raw), len(raw.columns))
            raw_written = to_excel_safe(raw, writer, sn, index=False)
            format_sheet(writer.sheets[sn], raw_written, hf, None, 0, (1, 0), 24)

        state_log = processing_log.assign(State_Workbook=state)
        state_log_written = to_excel_safe(state_log, writer, "Processing Log", index=False)
        format_sheet(writer.sheets["Processing Log"], state_log_written, hf, None, 0, (1, 0), 45)
...
def main():
    ...
    try:
        ...
        created = [p for s in states if (p := create_state_workbook(s, frames, processing_log, output_folder, spoc_map))]
        if not created:
